...
speed_test_encode_work(size_t len, int aead_mode)
{
    ...
    uint8_t *ad     = NULL;
    if (ad_len > 0) {
        ad = hiae_aligned_alloc(BUFFER_ALIGNMENT, ad_len);
        if (!ad) {
    ...
    uint8_t *msg = hiae_aligned_alloc(BUFFER_ALIGNMENT, len);
    uint8_t *ct  = hiae_aligned_alloc(BUFFER_ALIGNMENT, len + CRYPTO_ABYTES);
    if (!msg || !ct) {
...
speed_test_decode_work(size_t len, int aead_mode)
{
    ...
    uint8_t *msg = hiae_aligned_alloc(BUFFER_ALIGNMENT, len);
    uint8_t *ct  = hiae_aligned_alloc(BUFFER_ALIGNMENT, len + CRYPTO_ABYTES);
    uint8_t *dec = hiae_aligned_alloc(BUFFER_ALIGNMENT, len);
    if (!msg || !ct || !dec) {
        fprintf(stderr, "Failed to allocate memory\n");
        hiae_aligned_free(ad);
        hiae_aligned_free(msg);
