                  ...
                  const unsigned char *npub, const unsigned char *k)
{
    ...
    if (len > INT_MAX || npub == NULL || k == NULL || (len != 0 && (out == NULL || in == NULL))) {
        return -1;
    }
    if (len == 0) {
        return 0;
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_aes_128_ctr(), NULL, k, iv) != 1 ||
        EVP_EncryptUpdate(ctx, out, &written, in, (int) len) != 1 || written != (int) len) {
                    ...
                    unsigned long long adlen, const unsigned char *npub, const unsigned char *k)
{
    ...
    // a return value of zero or negative indicates a failure - the verification
    // failed.
...
int
crypto_aead_encrypt_bulk(EVP_CIPHER_CTX *ctx, unsigned char *c, unsigned long long *clen,
...
int
crypto_aead_decrypt_bulk(EVP_CIPHER_CTX *ctx, unsigned char *m, unsigned long long *mlen,
