static void
stream_xor(HiAEx2_state_t *state_opaque, uint8_t *ci, const uint8_t *mi, size_t size)
{
    HIAE_ALIGN(64) DATA256b state[STATE];
    const DATA256b          zero   = SIMD_ZERO_256();
    size_t                  i      = 0;
    size_t                  offset = 0;
...
#undef STREAM_BLOCK
    ...
    for (; size - i >= BLOCK_SIZE; i += BLOCK_SIZE, offset++) {
        const DATA256b block = enc_offset(state, zero, (int) offset);
