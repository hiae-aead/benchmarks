...
static inline DATA256b
SIMD_XOR3(const DATA256b a, const DATA256b b, const DATA256b c)
{
    DATA256b result;
...
{
    HIAE_ALIGN(64) DATA256b M[16], C[16];
...
static void
HiAEx2_init_arm_sha3(HiAEx2_state_t *state_opaque, const uint8_t *key, const uint8_t *nonce)
{
...
static void
HiAEx2_absorb_arm_sha3(HiAEx2_state_t *state_opaque, const uint8_t *ad, size_t len)
{
    HIAE_ALIGN(64) DATA256b state[STATE];
                         ...
                         uint8_t *tag)
{
    ...
    DATA256b acc_final = SIMD_XOR3(acc0, acc1, acc2);
...
static void
HiAEx2_dec_arm_sha3(HiAEx2_state_t *state_opaque, uint8_t *mi, const uint8_t *ci, size_t size)
{
    HIAE_ALIGN(64) DATA256b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    size_t rest   = size % UNROLL_BLOCK_SIZE;
                        ...
                        size_t msg_len, const uint8_t *ad, size_t ad_len, uint8_t *tag)
{
    ...
    HiAEx2_enc_arm_sha3(&state, ct, msg, msg_len);
    HiAEx2_finalize_arm_sha3(&state, ad_len, msg_len, tag);
                        ...
                        size_t ct_len, const uint8_t *ad, size_t ad_len, const uint8_t *tag)
{
    ...
    HiAEx2_finalize_arm_sha3(&state, ad_len, ct_len, computed_tag);
...
const HiAEx2_impl_t hiaex2_arm_sha3_impl = { .name         = "ARM SHA3",
                                             ...
                                             .dec_partial_noupdate =
                                                 HiAEx2_dec_partial_noupdate_arm_sha3,
                                             .encrypt = HiAEx2_encrypt_arm_sha3,
                                             .decrypt = HiAEx2_decrypt_arm_sha3,
                                             .mac     = HiAEx2_mac_arm_sha3 };
...
const HiAEx2_impl_t hiaex2_arm_sha3_impl = { .name                 = NULL,
                                             .init                 = NULL,
                                             .absorb               = NULL,
                                             .finalize             = NULL,
                                             .enc                  = NULL,
