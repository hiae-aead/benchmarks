...
/* For HiAEx2 with AVX+AESNI, we process 256-bit blocks as two 128-bit halves */
typedef struct {
    DATA128b low;
    DATA128b high;
} DATA256b;
...
static void
HiAEx2_init_aesni_avx(HiAEx2_state_t *state_opaque, const uint8_t *key, const uint8_t *nonce)
{
    ...
    init_update(state, tmp, c0, c1);

    memcpy(state_opaque->opaque, state, sizeof(state));
}

static void
HiAEx2_absorb_aesni_avx(HiAEx2_state_t *state_opaque, const uint8_t *ad, size_t len)
{
    HIAE_ALIGN(64) DATA256b state[STATE];
...
HiAEx2_finalize_mac_aesni_avx(HiAEx2_state_t *state_opaque, uint64_t data_len, uint8_t *tag)
{
    ...
    SIMD_STORE128(tag, SIMD_FOLD(tag_multi, tag_multi));

    memcpy(state_opaque->opaque, state, sizeof(state));
}

static void
HiAEx2_enc_aesni_avx(HiAEx2_state_t *state_opaque, uint8_t *ci, const uint8_t *mi, size_t size)
{
    HIAE_ALIGN(64) DATA256b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
...
static void
HiAEx2_dec_aesni_avx(HiAEx2_state_t *state_opaque, uint8_t *mi, const uint8_t *ci, size_t size)
{
    HIAE_ALIGN(64) DATA256b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    size_t rest   = size % UNROLL_BLOCK_SIZE;
...
static void
HiAEx2_enc_partial_noupdate_aesni_avx(HiAEx2_state_t *state_opaque,
                                      uint8_t        *ci,
                                      const uint8_t  *mi,
                                      size_t          size)
{
...
static void
HiAEx2_dec_partial_noupdate_aesni_avx(HiAEx2_state_t *state_opaque,
                                      uint8_t        *mi,
                                      const uint8_t  *ci,
                                      size_t          size)
{
                         ...
                         uint8_t       *tag)
{
    ...
    HiAEx2_enc_aesni_avx(&state, ct, msg, msg_len);
    HiAEx2_finalize_aesni_avx(&state, ad_len, msg_len, tag);
                         ...
                         const uint8_t *tag)
{
    ...
    HiAEx2_finalize_aesni_avx(&state, ad_len, ct_len, computed_tag);
    ...
    const uint8_t *key, const uint8_t *nonce, const uint8_t *data, size_t data_len, uint8_t *tag)
{
    HiAEx2_state_t state;
    HiAEx2_init_aesni_avx(&state, key, nonce);
    HiAEx2_absorb_aesni_avx(&state, data, data_len);
    HiAEx2_finalize_mac_aesni_avx(&state, data_len, tag);
