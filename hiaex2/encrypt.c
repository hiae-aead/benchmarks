                    ...
                    const unsigned char *nsec, const unsigned char *npub, const unsigned char *k)
{
    ...
    /* HiAEx2 outputs ciphertext and tag separately */
    uint8_t *tag = c + mlen;

    int ret = HiAEx2_encrypt(k, npub, m, c, mlen, ad, adlen, tag);
                    ...
                    unsigned long long adlen, const unsigned char *npub, const unsigned char *k)
{
    ...
    /* HiAEx2 expects ciphertext and tag separately */
    const uint8_t *tag = c + msg_len;

    int ret = HiAEx2_decrypt(k, npub, m, c, msg_len, ad, adlen, tag);
                  ...
                  const unsigned char *npub, const unsigned char *k)
{
    ...
    HiAEx2_init(&state, k, npub);
    HiAEx2_stream_xor(&state, out, in, (size_t) len);
    return 0;
}
