...
check_hex(const uint8_t *actual, size_t size, const char *expected, const char *name)
{
    ...
    for (size_t i = 0; i < size; i++) {
        if (hex[actual[i] >> 4] != expected[i * 2] || hex[actual[i] & 15] != expected[i * 2 + 1]) {
...
test_stream(size_t size, unsigned variant)
{
    ...
    for (size_t i = 0; i < sizeof key; i++)
        key[i] = (uint8_t) (i + variant);
    for (size_t i = 0; i < sizeof iv; i++)
        iv[i] = (uint8_t) (3 * i + variant);
    for (size_t i = 0; i < sizeof msg; i++)
        msg[i] = (uint8_t) (7 * i + variant + 1);
    if (((uintptr_t) ctx.key | (uintptr_t) ctx.state | (uintptr_t) key | (uintptr_t) iv |
         (uintptr_t) msg | (uintptr_t) ct) %
            64 !=
        0) {
    ...
    if (check_equal(tag, expected_tag, sizeof tag, "Zero-absorption tag"))
        return 1;

    rocca_init(&ctx, key, iv);
    rocca_stream_xor(&ctx, ct, ct, size);
    if (check_equal(ct, msg, size, "In-place stream decryption"))
        return 1;
    rocca_init(&ctx, key, iv);
    rocca_stream_xor(&ctx, ct, ct, size);
    if (check_equal(ct, expected, size, "In-place stream encryption"))
        return 1;
...
main(void)
{
    ...
    for (unsigned variant = 0; variant < 3; variant++) {
        for (size_t size = 0; size <= 129; size++) {
            if (test_stream(size, variant))
                return 1;
        }
        for (size_t i = 0; i < sizeof larger_sizes / sizeof larger_sizes[0]; i++) {
            if (test_stream(larger_sizes[i], variant))
                return 1;
        }
    }
