...
static inline void
AESROUND(unsigned char *out, unsigned char *in, unsigned char *rk)
{
    uint32_t out_C0 = TE0[*(in + 0)] ^ TE1[*(in + 5)] ^ TE2[*(in + 10)] ^ TE3[*(in + 15)] ^
                      BLK(rk[0], rk[1], rk[2], rk[3]);
    uint32_t out_C1 = TE0[*(in + 4)] ^ TE1[*(in + 9)] ^ TE2[*(in + 14)] ^ TE3[*(in + 3)] ^
                      BLK(rk[4], rk[5], rk[6], rk[7]);
    uint32_t out_C2 = TE0[*(in + 8)] ^ TE1[*(in + 13)] ^ TE2[*(in + 2)] ^ TE3[*(in + 7)] ^
                      BLK(rk[8], rk[9], rk[10], rk[11]);
    uint32_t out_C3 = TE0[*(in + 12)] ^ TE1[*(in + 1)] ^ TE2[*(in + 6)] ^ TE3[*(in + 11)] ^
                      BLK(rk[12], rk[13], rk[14], rk[15]);
    OUTPUT(out + 0, out_C0);
    OUTPUT(out + 4, out_C1);
    OUTPUT(out + 8, out_C2);
...
_MM_loadu_si128(const __M128I *mem_addr)
{
    __M128I a;
...
#define VARS4UPDATE ROCCA_ALIGN __m128i k[2], state[S_NUM], stateNew[S_NUM], M[M_NUM];
...
ROCCA_ALIGN static const uint8_t Z0[] = { 0xcd, 0x65, 0xef, 0x23, 0x91, 0x44, 0x37, 0x71,
                                          0x22, 0xae, 0x28, 0xd7, 0x98, 0x2f, 0x8a, 0x42 };
ROCCA_ALIGN static const uint8_t Z1[] = { 0xbc, 0xdb, 0x89, 0x81, 0xa5, 0xdb, 0xb5, 0xe9,
                                          0x2f, 0x3b, 0x4d, 0xec, 0xcf, 0xfb, 0xc0, 0xb5 };

void
rocca_init(rocca_context *ctx, const uint8_t *key, const uint8_t *iv)
{
    VARS4UPDATE
    INIT_STATE(key, iv);
...
void
rocca_add_ad(rocca_context *ctx, const uint8_t *in, size_t size)
{
    VARS4UPDATE
    COPY_TO_LOCAL(ctx);
    size_t i = 0;
...
void
rocca_encrypt(rocca_context *ctx, uint8_t *out, const uint8_t *in, size_t size)
{
    VARS4ENCRYPT
    COPY_TO_LOCAL(ctx);
    size_t i = 0;
...
void
rocca_decrypt(rocca_context *ctx, uint8_t *out, const uint8_t *in, size_t size)
{
    VARS4ENCRYPT
    COPY_TO_LOCAL(ctx);
    size_t i = 0;
...
void
rocca_stream_xor(rocca_context *ctx, uint8_t *out, const uint8_t *in, size_t size)
{
...
void
rocca_tag(rocca_context *ctx, uint8_t *tag)
{
    VARS4UPDATE
