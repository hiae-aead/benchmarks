       ...
       const unsigned char *nonce, const unsigned char *key, unsigned char *decrypted)
{
    ...
    CHECK(crypto_aead_decrypt(decrypted, &mlen, NULL, ciphertext, clen, ad, adlen, nonce, key) ==
          -1);
    CHECK(mlen == 0 && is_zero(decrypted, length));
    CHECK(guards_intact(decrypted, length));
    mlen = 123;
    CHECK(crypto_aead_decrypt(NULL, &mlen, NULL, ciphertext, clen, ad, adlen, nonce, key) == -1);
    CHECK(mlen == 0);
    return 0;
}
...
test_vector(const unsigned char *key, const unsigned char *nonce)
{
    ...
    CHECK(crypto_aead_encrypt(ciphertext, &clen, message, sizeof message, ad, sizeof ad, NULL,
                              nonce, key) == 0);
    ...
    CHECK(crypto_aead_decrypt(decrypted, &mlen, NULL, ciphertext, clen, ad, sizeof ad, nonce,
                              key) == 0);
...
static int
test_lengths(size_t length, size_t adlen, const char *expected_tag, const unsigned char *key,
             const unsigned char *nonce)
{
    ...
    CHECK(crypto_aead_encrypt(ciphertext, &clen, message, length, ad, adlen, NULL, nonce, key) ==
          0);
    CHECK(clen == capacity);
    ...
    CHECK(crypto_aead_decrypt(NULL, &mlen, NULL, ciphertext, clen, ad, adlen, nonce, key) == 0);
    CHECK(mlen == length);
    memcpy(in_place, message, length);
    CHECK(crypto_aead_encrypt(in_place, NULL, in_place, length, ad, adlen, NULL, nonce, key) == 0);
    CHECK(memcmp(in_place, ciphertext, capacity) == 0);
    CHECK(crypto_aead_decrypt(in_place, NULL, NULL, in_place, capacity, ad, adlen, nonce, key) ==
          0);
    ...
    mlen = 123;
    CHECK(crypto_aead_decrypt(in_place, &mlen, NULL, in_place, capacity, ad, adlen, nonce, key) ==
          -1);
...
test_short_ciphertext(const unsigned char *key, const unsigned char *nonce)
{
    ...
    for (length = 0; length < CRYPTO_ABYTES; length++) {
        mlen = 123;
        CHECK(crypto_aead_decrypt(&decrypted, &mlen, NULL, ciphertext, length, NULL, 0, nonce,
                                  key) == -1);
...
main(void)
{
    ...
    for (i = 0; i < count; i++) {
        for (j = 0; j < count; j++) {
            if (test_lengths(vectors[i].length, vectors[j].length, i == j ? vectors[i].tag : NULL,
                             key, nonce) != 0) {
