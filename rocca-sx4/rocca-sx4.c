...
// AVX-512 optimized path for ROCCA-SX4
#define load(a)     _mm512_loadu_si512((const void *) (a))
#define load128(a)  _mm512_broadcast_i64x2(_mm_loadu_si128((const void *) (a)))
#define store(a, b) _mm512_storeu_si512((void *) (a), (b))
#define store128(a, b) _mm_storeu_si128((__m128i *) (a), fold(b))
#define xor(a,b) _mm512_xor_si512(a, b)
#define and(a,b) _mm512_and_si512(a, b)
#define setzero() _mm512_setzero_si512()
#define enc(a,b) _mm512_aesenc_epi128(a,b)
#define fold(x) _mm_xor_si128(_mm_xor_si128(_mm512_extracti64x2_epi64(x, 0), _mm512_extracti64x2_epi64(x, 1)), \
                               _mm_xor_si128(_mm512_extracti64x2_epi64(x, 2), _mm512_extracti64x2_epi64(x, 3)))

#elif defined(__VAES__) && defined(__AVX2__)

// AVX2+VAES optimized path - operates on 512-bit using two 256-bit operations
#define load(a) load_avx2_512(a)
#define load128(a) load128_avx2_512(a)
#define store(a,b) store_avx2_512(a,b)
#define store128(a, b) _mm_storeu_si128((void *)a, fold(b))
#define fold(a) fold_avx2_512(a)
#define xor(a,b) xor_avx2_512(a,b)
#define and(a,b) and_avx2_512(a,b)
#define setzero() setzero_avx2_512()
#define enc(a,b) enc_avx2_512(a,b)

// Type definition for 512-bit emulation using two 256-bit values
typedef struct { __m256i lo; __m256i hi; } __m512i;

static inline __m512i load_avx2_512(const uint8_t *m) {
    return (__m512i) {
        _mm256_loadu_si256((const __m256i *)(m)),
        _mm256_loadu_si256((const __m256i *)(m + 32))
    };
}

static inline __m512i load128_avx2_512(const uint8_t *m) {
    const __m128i z = _mm_loadu_si128((const __m128i *)(m));
    const __m256i z256 = _mm256_broadcastsi128_si256(z);
    return (__m512i) { z256, z256 };
}

static inline void store_avx2_512(uint8_t *m, const __m512i a) {
    _mm256_storeu_si256((__m256i *)(m), a.lo);
    _mm256_storeu_si256((__m256i *)(m + 32), a.hi);
}

static inline __m128i fold_avx2_512(const __m512i a) {
    const __m256i combined = _mm256_xor_si256(a.lo, a.hi);
    return _mm_xor_si128(_mm256_castsi256_si128(combined), 
                         _mm256_extractf128_si256(combined, 1));
}

static inline __m512i xor_avx2_512(const __m512i a, const __m512i b) {
    return (__m512i) {
        _mm256_xor_si256(a.lo, b.lo),
        _mm256_xor_si256(a.hi, b.hi)
    };
}

static inline __m512i and_avx2_512(const __m512i a, const __m512i b) {
    return (__m512i) {
        _mm256_and_si256(a.lo, b.lo),
        _mm256_and_si256(a.hi, b.hi)
    };
}

static inline __m512i setzero_avx2_512(void) {
    return (__m512i) {
...
static inline __m512i enc_avx2_512(const __m512i a, const __m512i b) {
    return (__m512i) {
        _mm256_aesenc_epi128(a.lo, b.lo),
...
#define __m512i rocca__m512i

typedef struct { __m128i a; __m128i b; __m128i c; __m128i d; } __m512i;

static inline __m512i load(const uint8_t *m)  {
    return (__m512i) {
        _mm_loadu_si128((const __m128i *)(m)),
        _mm_loadu_si128((const __m128i *)(m + 16)),
        _mm_loadu_si128((const __m128i *)(m + 32)),
        _mm_loadu_si128((const __m128i *)(m + 48))
    };
}

static inline __m512i load128(const uint8_t *m)  {
    const __m128i z =  _mm_loadu_si128((const __m128i *)(m));
    return (__m512i) {
        z, z, z, z
    };
...
static inline __m128i fold(const __m512i x)  {
    return _mm_xor_si128(_mm_xor_si128(x.a,x.b), _mm_xor_si128(x.c,x.d));
}

static inline void store(uint8_t *m, const __m512i a) {
    _mm_storeu_si128((__m128i *)(m),a.a);
    _mm_storeu_si128((__m128i *)(m + 16),a.b);
    _mm_storeu_si128((__m128i *)(m + 32),a.c);
    _mm_storeu_si128((__m128i *)(m + 48),a.d);
}

static inline void store128(uint8_t *m, const __m128i a) {
    _mm_storeu_si128((__m128i *)(m),a);
}

static inline __m512i xor(const __m512i a, const __m512i b) {
    return (__m512i) {
        _mm_xor_si128(a.a,b.a),
        _mm_xor_si128(a.b,b.b),
        _mm_xor_si128(a.c,b.c),
        _mm_xor_si128(a.d,b.d)
    };
}

static inline __m512i and(const __m512i a, const __m512i b) {
    return (__m512i) {
        _mm_and_si128(a.a,b.a),
        _mm_and_si128(a.b,b.b),
        _mm_and_si128(a.c,b.c),
        _mm_and_si128(a.d,b.d)
    };
}


static inline __m512i enc(const __m512i a, const __m512i b) {
   return (__m512i) {   
       _mm_aesenc_si128(a.a,b.a),
       _mm_aesenc_si128(a.b,b.b),
       _mm_aesenc_si128(a.c,b.c),
       _mm_aesenc_si128(a.d,b.d)
   };
}

static inline __m512i setzero(void) {
    return (__m512i) {   
        _mm_setzero_si128(),
        _mm_setzero_si128(),
        _mm_setzero_si128(),
...
#define VARS4UPDATE \
  __m512i k[2], state[S_NUM], stateNew[S_NUM], M[M_NUM];

#define VARS4ENCRYPT \
  VARS4UPDATE \
  __m512i Z[M_NUM], C[M_NUM];
...
#define XOR_BLOCK(dst, src1, src2) \
  ...
  dst[2] = xor(src1[2], src2[2]); \
  dst[3] = xor(src1[3], src2[3]); 

#define MASKXOR_BLOCK(dst, src1, src2, mask) \
  dst[0] = and(xor(src1[0], src2[0]), mask[0]); \
  dst[1] = and(xor(src1[1], src2[1]), mask[1]); \
  dst[2] = and(xor(src1[2], src2[2]), mask[2]); \
  dst[3] = and(xor(src1[3], src2[3]), mask[3]);
...
#define MAKE_TAG(sizeAD, sizeM, tag) \
  ...
  __m512i tag128a = setzero(); \
  for(size_t i = 0; i <= 3; ++i) { \
    tag128a = xor(tag128a, state[i]); \
  } \
  __m512i tag128b = setzero(); \
  for(size_t i = 4; i <= 6; ++i) { \
    tag128b = xor(tag128b, state[i]); \
  } \
  store128((tag)   , tag128a); \
  store128((tag)+16, tag128b);
