...
static inline void
aegis_timer_start(aegis_timer_t *timer)
{
    timer->has_cycles = aegis_has_cycle_counter();
    if (timer->has_cycles) {
        timer->start_cycles = aegis_read_cycles();
        timer->end_cycles   = timer->start_cycles;
    }
    timer->start_time = aegis_get_time();
    timer->end_time   = timer->start_time;
}

static inline void
aegis_timer_stop(aegis_timer_t *timer)
{
    timer->end_time = aegis_get_time();
    if (timer->has_cycles) {
        timer->end_cycles = aegis_read_cycles();
    }
}

static inline double
aegis_timer_elapsed_seconds(const aegis_timer_t *timer)
{
    return timer->end_time - timer->start_time;
}

static inline uint64_t
aegis_timer_elapsed_cycles(const aegis_timer_t *timer)
{
    if (!timer->has_cycles)
        return 0;
...
aegis_get_cpu_frequency(void)
{
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        double start = aegis_get_time();

        while ((aegis_get_time() - start) < target_time) {
#ifdef _MSC_VER
            _ReadWriteBarrier();
#else
        ...
        uint64_t cycles  = aegis_timer_elapsed_cycles(&timer);
...
static inline aegis_stats_t *
aegis_stats_create(size_t initial_capacity)
{
    aegis_stats_t *stats = (aegis_stats_t *) malloc(sizeof(aegis_stats_t));
    if (!stats)
        return NULL;
...
static inline void
aegis_stats_destroy(aegis_stats_t *stats)
{
    if (stats) {
        free(stats->values);
        free(stats);
...
static inline void
aegis_stats_add(aegis_stats_t *stats, double value)
{
...
static inline void
aegis_stats_compute(aegis_stats_t *stats)
{
    if (stats->count == 0)
        return;
...
static inline void *
aegis_aligned_alloc(size_t alignment, size_t size)
{
#ifdef _WIN32
