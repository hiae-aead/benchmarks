...
SIMD_FOLD(const DATA512b x)
{
    ...
    result = veorq_u8(result, x.data[2]);
...
{
    DATA512b temp = XAESL(state[(P_0 + offset) % STATE], state[(P_1 + offset) % STATE]);
...
static void
HiAEx4_init_arm(HiAEx4_state_t *state_opaque, const uint8_t *key, const uint8_t *nonce)
{
...
static void
HiAEx4_absorb_arm(HiAEx4_state_t *state_opaque, const uint8_t *ad, size_t len)
{
    HIAE_ALIGN(64) DATA512b state[STATE];
...
static void
HiAEx4_finalize_arm(HiAEx4_state_t *state_opaque, uint64_t ad_len, uint64_t msg_len, uint8_t *tag)
{
    HIAE_ALIGN(64) DATA512b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
 ...
 * implementations */
static void
HiAEx4_finalize_mac_arm(HiAEx4_state_t *state_opaque, uint64_t data_len, uint8_t *tag)
{
    HIAE_ALIGN(64) DATA512b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    const uint8_t degree = 4;
...
static void
HiAEx4_enc_arm(HiAEx4_state_t *state_opaque, uint8_t *ci, const uint8_t *mi, size_t size)
{
    HIAE_ALIGN(64) DATA512b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
...
static void
HiAEx4_enc_partial_noupdate_arm(HiAEx4_state_t *state_opaque, uint8_t *ci, const uint8_t *mi,
                                size_t size)
{
    if (size == 0)
        return;
...
static void
HiAEx4_dec_partial_noupdate_arm(HiAEx4_state_t *state_opaque, uint8_t *mi, const uint8_t *ci,
                                size_t size)
{
    if (size == 0)
        return;
...
static void
HiAEx4_dec_arm(HiAEx4_state_t *state_opaque, uint8_t *mi, const uint8_t *ci, size_t size)
{
    HIAE_ALIGN(64) DATA512b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    size_t rest   = size % UNROLL_BLOCK_SIZE;
...
static int
HiAEx4_encrypt_arm(const uint8_t *key, const uint8_t *nonce, const uint8_t *msg, uint8_t *ct,
                   size_t msg_len, const uint8_t *ad, size_t ad_len, uint8_t *tag)
{
    HiAEx4_state_t state;
    HiAEx4_init_arm(&state, key, nonce);
    HiAEx4_absorb_arm(&state, ad, ad_len);
    HiAEx4_enc_arm(&state, ct, msg, msg_len);
    HiAEx4_finalize_arm(&state, ad_len, msg_len, tag);

    return 0;
}

static int
HiAEx4_decrypt_arm(const uint8_t *key, const uint8_t *nonce, uint8_t *msg, const uint8_t *ct,
                   size_t ct_len, const uint8_t *ad, size_t ad_len, const uint8_t *tag)
{
    ...
    HiAEx4_init_arm(&state, key, nonce);
    HiAEx4_absorb_arm(&state, ad, ad_len);
    HiAEx4_dec_arm(&state, msg, ct, ct_len);
    HiAEx4_finalize_arm(&state, ad_len, ct_len, computed_tag);

    return hiaex4_constant_time_compare(computed_tag, tag, HIAEX4_MACBYTES);
}

static int
HiAEx4_mac_arm(const uint8_t *key, const uint8_t *nonce, const uint8_t *data, size_t data_len,
               uint8_t *tag)
{
    HiAEx4_state_t state;
    HiAEx4_init_arm(&state, key, nonce);
    HiAEx4_absorb_arm(&state, data, data_len);
    HiAEx4_finalize_mac_arm(&state, data_len, tag);
...
const HiAEx4_impl_t hiaex4_arm_impl = { .name                 = "ARM NEON",
                                        .init                 = HiAEx4_init_arm,
                                        ...
                                        .dec_partial_noupdate = HiAEx4_dec_partial_noupdate_arm,
                                        .encrypt              = HiAEx4_encrypt_arm,
                                        .decrypt              = HiAEx4_decrypt_arm,
                                        .mac                  = HiAEx4_mac_arm };
...
const HiAEx4_impl_t hiaex4_arm_impl = { .name                 = NULL,
                                        .init                 = NULL,
                                        .absorb               = NULL,
                                        .finalize             = NULL,
                                        .finalize_mac         = NULL,
