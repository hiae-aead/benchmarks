 ...
 *
 * HiAEx4 uses 128-bit (16-byte) nonces (initialization vectors).
 ...
 * Example:
 * @code
 * uint8_t key[HIAEX4_KEYBYTES];
 * uint8_t nonce[HIAEX4_NONCEBYTES];
 * uint8_t plaintext[] = "Secret message";
 * uint8_t ciphertext[sizeof(plaintext)];
 * uint8_t tag[HIAEX4_MACBYTES];
 * uint8_t ad[] = "metadata";
 *
 * // Generate random key and nonce (use proper CSPRNG)
 * // ...
 *
 * int ret = HiAEx4_encrypt(key, nonce, plaintext, ciphertext,
 ...
 * @endcode
 */
int HiAEx4_encrypt(const uint8_t *key, const uint8_t *nonce, const uint8_t *msg, uint8_t *ct,
 ...
 *
 * int ret = HiAEx4_decrypt(key, nonce, decrypted, ciphertext,
 ...
 * @endcode
 */
int HiAEx4_decrypt(const uint8_t *key, const uint8_t *nonce, uint8_t *msg, const uint8_t *ct,
 ...
 *
 * @param key       Authentication key (must be HIAEX4_KEYBYTES bytes)
 ...
 * @return 0 on success, non-zero on error
 *
 * @note This is equivalent to HiAEx4_encrypt with a zero-length message
 ...
 *
 * int ret = HiAEx4_mac(key, nonce, metadata, sizeof(metadata), tag);
 * @endcode
 */
int HiAEx4_mac(const uint8_t *key, const uint8_t *nonce, const uint8_t *data, size_t data_len,
               uint8_t *tag);
...
/**
 * @brief Initialize the HiAEx4 library
 *
 ...
 * if (HiAEx4_force_implementation("Software") != 0) {
 ...
 * @endcode
 */
int HiAEx4_force_implementation(const char *impl_name);
 ...
 *
 * @param expected_tag  The expected tag (HIAEX4_MACBYTES bytes)
 * @param actual_tag    The tag to verify (HIAEX4_MACBYTES bytes)
 *
 * @return 0 if tags match, non-zero if different
 *
 * @note Always compares exactly HIAEX4_MACBYTES bytes
 * @note Execution time is independent of tag contents
 *
 * Example:
 * @code
 * if (HiAEx4_verify_tag(expected_tag, computed_tag) != 0) {
 *     // Authentication failed
 * }
 * @endcode
 */
int HiAEx4_verify_tag(const uint8_t *expected_tag, const uint8_t *actual_tag);
 ...
 */
typedef enum {
    HiAEx4_STREAM_INIT  = 0, /**< Initial state after HiAEx4_stream_init() */
    HiAEx4_STREAM_AD    = 1, /**< Processing associated data */
    HiAEx4_STREAM_MSG   = 2, /**< Processing message data (encrypt/decrypt) */
    HiAEx4_STREAM_FINAL = 3 /**< Finalized, tag generated/verified */
} HiAEx4_stream_phase_t;
 ...
 */
typedef enum {
    HiAEx4_STREAM_MODE_NONE    = 0, /**< Not yet determined */
    HiAEx4_STREAM_MODE_ENCRYPT = 1, /**< Encryption mode */
    HiAEx4_STREAM_MODE_DECRYPT = 2 /**< Decryption mode */
} HiAEx4_stream_mode_t;
 ...
 * @param key     Encryption/decryption key (HIAEX4_KEYBYTES bytes)
 * @param nonce   Unique nonce for this operation (HIAEX4_NONCEBYTES bytes)
 *
 ...
 * @endcode
 */
void HiAEx4_stream_init(HiAEx4_stream_state_t *stream, const uint8_t *key, const uint8_t *nonce);
 ...
 * Example:
 * @code
 * HiAEx4_stream_absorb(&stream, header1, 100);
 * HiAEx4_stream_absorb(&stream, header2, 57);
 * @endcode
 */
void HiAEx4_stream_absorb(HiAEx4_stream_state_t *stream, const uint8_t *ad, size_t ad_len);
 ...
 *     HiAEx4_stream_encrypt(&stream, output, buffer, bytes);
 *     fwrite(output, 1, bytes, output_file);
 * }
 * @endcode
 */
void HiAEx4_stream_encrypt(HiAEx4_stream_state_t *stream, uint8_t *ct, const uint8_t *pt,
                           size_t len);
 ...
 * @note Does not verify authentication until HiAEx4_stream_verify()
 */
void HiAEx4_stream_decrypt(HiAEx4_stream_state_t *stream, uint8_t *pt, const uint8_t *ct,
                           size_t len);
 ...
 *
 * @param stream  Stream state
 * @param tag     Output buffer for authentication tag (HIAEX4_MACBYTES bytes)
 *
 * @note Stream cannot be used after finalization
 * @note For decryption, use HiAEx4_stream_verify() instead
 *
 * Example:
 * @code
 * uint8_t tag[HIAEX4_MACBYTES];
 * HiAEx4_stream_finalize(&stream, tag);
 * // Save or transmit tag with ciphertext
 * @endcode
 */
void HiAEx4_stream_finalize(HiAEx4_stream_state_t *stream, uint8_t *tag);
 ...
 * Example:
 * @code
 * if (HiAEx4_stream_verify(&stream, received_tag) != 0) {
 *     // Authentication failed - data was tampered with
 *     // Discard all decrypted data
 * }
 * @endcode
 */
int HiAEx4_stream_verify(HiAEx4_stream_state_t *stream, const uint8_t *expected_tag);
 ...
 * the last must process multiples of 64 bytes (HiAEx4's block size).
 *
 ...
 */
void HiAEx4_init(HiAEx4_state_t *state, const uint8_t *key, const uint8_t *nonce);
 ...
 *
 * @warning All calls except the last must have len as multiple of 64
 * @note Must complete all AD before any enc/dec operations
 *
 * Example:
 * @code
 * HiAEx4_absorb(&state, ad_chunk1, 128);   // Multiple of 64
 * HiAEx4_absorb(&state, ad_chunk2, 64);    // Multiple of 64
 * HiAEx4_absorb(&state, ad_chunk3, 7);     // Last chunk - any size
 * @endcode
 */
void HiAEx4_absorb(HiAEx4_state_t *state, const uint8_t *ad, size_t len);
 ...
 *
 * @warning All calls except the last must have size as multiple of 64
 * @note Output buffer must be at least 'size' bytes
 *
 * Example:
 * @code
 * HiAEx4_enc(&state, ct1, pt1, 512);  // Multiple of 64
 * HiAEx4_enc(&state, ct2, pt2, 128);  // Multiple of 64
 * HiAEx4_enc(&state, ct3, pt3, 13);   // Last chunk - any size
 * @endcode
 */
 ...
 */
void HiAEx4_dec(HiAEx4_state_t *state, uint8_t *mi, const uint8_t *ci, size_t size);
 ...
 * @param ci     Output ciphertext buffer
 * @param mi     Input plaintext
 * @param size   Number of bytes (must be < 64)
 *
 * @warning Only use for partial blocks < 64 bytes
 ...
 * HiAEx4_enc_partial_noupdate(&state, ct_partial, buffer, 7);
 * // ... when more data arrives to complete block ...
 * memcpy(buffer + 7, more_data, 57);  // Now have full 64-byte block
 * HiAEx4_enc(&state, ct_full, buffer, 64);  // Updates state
 * @endcode
 */
void HiAEx4_enc_partial_noupdate(HiAEx4_state_t *state, uint8_t *ci, const uint8_t *mi,
                                 size_t size);
 ...
 * @warning Must eventually process more data to update state properly
 */
void HiAEx4_dec_partial_noupdate(HiAEx4_state_t *state, uint8_t *mi, const uint8_t *ci,
                                 size_t size);
