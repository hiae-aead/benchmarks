...
#define load128(a)  _mm256_broadcastsi128_si256(_mm_loadu_si128((const void *) (a)))
#define load(a) _mm256_loadu_si256((const void *) a)
#define store(a,b) _mm256_storeu_si256((void *)a, b)
#define store128(a, b) _mm_storeu_si128((void *)a, b)
#define fold(a) _mm_xor_si128(_mm256_castsi256_si128(a), _mm256_extractf128_si256(a, 1))
#define xor(a,b) _mm256_xor_si256(a, b)
#define and(a,b) _mm256_and_si256(a, b)
#define setzero() _mm256_setzero_si256()
#define enc(a,b) _mm256_aesenc_epi128(a,b)

#else

#define __m256i rocca__m256i

typedef struct { __m128i a; __m128i b; } __m256i;

static inline __m256i load(const uint8_t *m)  {
    return (__m256i) {
...
static inline __m256i load128(const uint8_t *m)  {
...
static inline __m128i fold(const __m256i x)  {
    return _mm_xor_si128(x.a,x.b);
}

static inline void store(uint8_t *m, const __m256i a) {
...
static inline __m256i xor(const __m256i a, const __m256i b) {
    return (__m256i) {
...
static inline __m256i and(const __m256i a, const __m256i b) {
    return (__m256i) {
...
static inline __m256i enc(const __m256i a, const __m256i b) {
   return (__m256i) {   
...
static inline __m256i setzero(void) {
    return (__m256i) {   
        _mm_setzero_si128(),
        _mm_setzero_si128()
    };
...
#define VARS4UPDATE \
  __m256i k[2], state[S_NUM], stateNew[S_NUM], M[M_NUM];

#define VARS4ENCRYPT \
  VARS4UPDATE \
  __m256i Z[M_NUM], C[M_NUM];
...
#define MAKE_TAG(sizeAD, sizeM, tag) \
  ...
  __m256i tag128a = setzero(); \
  for(size_t i = 0; i <= 3; ++i) { \
    tag128a = xor(tag128a, state[i]); \
  } \
  __m256i tag128b = setzero(); \
  for(size_t i = 4; i <= 6; ++i) { \
    tag128b = xor(tag128b, state[i]); \
  } \
  store128((tag)   , fold(tag128a)); \
  store128((tag)+16, fold(tag128b));
